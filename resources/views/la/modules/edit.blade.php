...
@section("contentheader_title", trans("admin.Edit course: "))
@section("contentheader_description", $course->$view_col)
@section("section", trans("admin.Courses"))
			...
			<div class="col-md-8 col-md-offset-2">
				{!! Form::model($course, ['route' => [config('laraadmin.adminRoute') . '.modules.update', $course->id ], 'method'=>'PUT', 'id' => 'course-edit-form']) !!}
					...
					<div class="form-group">
						{!! Form::submit( trans("admin.Update"), ['class'=>'btn btn-success']) !!} <button class="btn btn-default pull-right"><a href="{{ url(config('laraadmin.adminRoute') . '/courses') }}">{{ trans('admin.Cancel') }}</a></button>
					</div>
				{!! Form::close() !!}
