...
class RolesController extends Controller
{
	...
	public function index()
	{
		...
		// return $all_childerns_ids= $this->getAllChildRolesIds(auth()->user()->roles[0]->id);
		// $values = DB::table('roles')->select($this->listing_cols)->WhereIn('id',$all_childerns_ids)->whereNull('deleted_at');
		...
		// return $this->getAllChildRoles(auth()->user()->roles[0]->id);
	...
	public function getAllChildRoles($parentRoleId)
    {
        ...
        $parentRole = MyRole::find($parentRoleId);
	...
	public function getAllChildRolesIds($parentRoleId)
    {
        // Retrieve the parent role and its descendants recursively
        $parentRole = MyRole::find($parentRoleId);
        ...
        if ($parentRole) {
            // Use a recursive function to get all descendants
            $roles = $this->getAllDescendantsIds($parentRole);
	...
	protected function getAllDescendantsIds($role)
    {
        $descendants = [];
        ...
        foreach ($children as $child) {
            ...
            $descendants = array_merge($descendants, $this->getAllDescendantsIds($child));
        }
		...
		if(Module::hasAccess("Roles", "delete")) {
                    ...
                    if(count($role->users)>0)
                    {
                        if($message)
                            ...
                            $message = "couldn`t be deleted there is related (users)";
                    }
                    if($message)
                    {
	...
	public function dtajax()
	{
		if(auth()->user()->roles[0]->parent==0){
			...
			$all_childerns_ids= $this->getAllChildRolesIds(auth()->user()->roles[0]->id);
			$values = DB::table('roles')->select($this->listing_cols)->WhereIn('id',$all_childerns_ids)->whereNull('deleted_at');
		}
		...
		for($i=0; $i < count($data->data); $i++) {
			...
			if($this->show_action) {
				$output = '';
				if(Module::hasAccess("Roles", "edit")) {
					$output .= '<a href="'.url(config('laraadmin.adminRoute') . '/roles/'.$data->data[$i]->id.'/edit').'" class="btn btn-warning btn-xs" style=""><i class="fa fa-edit"></i></a>';
				}
