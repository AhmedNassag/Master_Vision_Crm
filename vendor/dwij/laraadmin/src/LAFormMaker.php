	...
	**/
	public static function input($module, $field_name, $default_val = null, $required2 = null, $class = 'form-control', $params = [])
	{
		...
		if(Module::hasFieldAccess($module->id, $module->fields[$field_name]['id'], $access_type = "write")) {
			...
			$label = trans('admin.'.$module->fields[$field_name]['label']);
			...
			if(!isset($params['placeholder'])) {
				$params['placeholder'] = __("admin.Enter")." ".$label;
			}
			if($minlength) {
			...
			switch ($field_type->name) {
				...
				case 'File':
					...
					if(isset($upload->id)) {
						$out .= //"<a class='btn btn-default btn_upload_file hide' file_type='file' selecter='".$field_name."'>Upload <i class='fa fa-cloud-upload'></i></a>".
                                                    "<input type='file' id='".$field_name."_upload_files' name='".$field_name."_input' />
							<a class='uploaded_file' target='_blank' href='".url("files/".$upload->hash.DIRECTORY_SEPARATOR.$upload->name)."'><i class='fa fa-file-o'></i><i title='Remove File' class='fa fa-times'></i></a>";
                                                ...
                                                $out .= "<input type='file' id='".$field_name."_upload_files' name='".$field_name."_input' />";
				...
				case 'Files':
					...
					if(is_array($default_val_arr) && count($default_val_arr) > 0) {
						...
						foreach ($default_val_arr as $uploadId) {
							...
							if(isset($upload->id)) {
								$uploadIds[] = $upload->id;
								$fileImage = "";
								if(in_array(strtolower($upload->extension), ["jpg", "png", "gif", "jpeg","webp"])) {
					...
					$out .= "<input type='file' id='".$field_name."_upload_files' name='".$field_name."_input[]' multiple />";
					...
					if(isset($upload->id)) {
						$out .= "<input type='file' id='".$field_name."_upload_files' name='".$field_name."_input' />".
							...
							"<div class='uploaded_image'><img src='".url("files/".$upload->hash.DIRECTORY_SEPARATOR.$upload->name)."' width='150'><i title='Remove Image' class='fa fa-times'></i></div>";
                                            ...
                                            $out .= "<input type='file' ".$req_str." id='".$field_name."_upload_files' name='".$field_name."_input' />";
				...
				case 'Multiselect':
					...
					unset($params['data-rule-maxlength']);
					$params['data-placeholder'] = __("admin.Select multiple")." ".$label;//Str::plural($label);
					unset($params['placeholder']);
				...
				case 'Radio':
					...
					if(Str::startsWith($popup_vals, "@")) {
						...
						foreach ($popup_vals as $value) {
							...
							$out .= '<label>'.(Form::radio($field_name, $value, $sel)).' '. __("admin.".$value).' </label>';
						}
						$out .= '</div>';
						break;
					}
				...
				case 'Taginput':
					...
					$params['multiple'] = "true";
					$params['rel'] = "taginput";
					$params['data-placeholder'] = __("admin.Add multiple")." ".$label;
					unset($params['placeholder']);
	...
	public static function display($module, $field_name, $class = 'form-control')
	{
		...
		if(Module::hasFieldAccess($module->id, $module->fields[$field_name]['id'], $access_type = "view")) {
			...
			switch ($field_type->name) {
				...
				case 'Checkbox':
					if($value == 0) {
						$value = "<div class='label label-danger'>".__('admin.False')."</div>";
					} else {
						$value = "<div class='label label-success'>".__('admin.True')."</div>";
					}
					break;
				case 'Currency':
						...
						foreach ($uploads as $uploadId) {
							...
							if(isset($upload->id)) {
								...
								$uploads_html .= '<a class="preview" target="_blank" href="'.url("files/".$upload->hash.DIRECTORY_SEPARATOR.$upload->name).'" data-toggle="tooltip" data-placement="top" data-container="body" style="display:inline-block;margin-right:5px;" title="'.$upload->name.'">
										'.$fileImage.'</a>';
								$uploads_html .= "<a upload='".json_encode($u)."' class='edit-display-image id-".$upload->id."'>".__('admin.Edit')."</a>";
							}
						}
						$value = $uploads_html;
					} else {
				...
				case 'Image':
					...
					if(isset($upload->id)) {
						...
						$value .= "<a upload='".json_encode($u)."' class='edit-display-image id-".$upload->id."'>edit</a>";
					} else {
